...
class designation extends CI_Controller {
	...
	function delete($id = 0)
	{
		...
		$this->m_common->_delete('designation',array('id'=>$id));
	...
	function add()
	{
		...
		$this->form_validation->set_rules('designation', 'designation', 'required|is_unique[designation.designation]');
		
		$defaults = array ('designation'=>'',
						   'designation_short'=>'',
		...
		if($_POST)
		{
		   ...
		   else
		   {
			   $data = array('designation' => $this->input->post('designation'),
							 'designation_short' => $this->input->post('designation_short'),
                               
							 );
			   
			   $id = $this->m_common->_insert('designation',$data);
		...
		$this->load->view('includes/header', $this->data);
		$this->load->view('includes/sidebar', $this->data);
		$this->load->view('designation/form', $this->data);
		$this->load->view('includes/footer', $this->data);
	}
	...
	function edit($id = 0)
	{
		...
		$this->form_validation->set_rules('designation', 'designation', 'required|callback__check_name['.$id.']');
		...
		if($_POST)
		{
		   if (!$this->form_validation->run())
		   {
			   $this->data['row'] = array_merge($this->m_common->_get('designation',array('id'=>$id)), $_POST);
		   }
		   else
		   {
			   $data = array('designation' => $this->input->post('designation'),
			   ...
			   $id = $this->m_common->_update('designation',$data,array('id'=>$id));
			   
			   $this->session->set_flashdata('success', "designation <a href='".site_url('designation/edit/'.$id)."'>".$this->input->post('designation')." </a> Updated Successfully!");
		...
		{
		   $this->data['row'] = $this->m_common->_get('designation',array('id'=>$id));
		}
	...
	function get()
	{
		...
		$aColumns = array('id','designation','designation_short');
		...
		if($_GET['sSearch'] != '')
			$this->db->like('designation',$_GET['sSearch'],'both');
		...
		foreach ($result as $row) {
			if($this->session->userdata('roles_update')!=1) 
                   ...
                   {
                        $hrefval=site_url("designation/edit/".$row->id); 
                   }
	...
	{
		$type = $this->m_common->_get('designation',array('id <>'=>$id,'designation'=>$name));
